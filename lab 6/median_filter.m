function filteredImage = median_filter(inputImage, filterSize)
    ...
    for i = 1:rows
        for j = 1:cols
            ...
            sortedValues = sort(neighborhoodArray);
            medianIndex = floor(numel(sortedValues) / 2) + 1;
            medianValue = sortedValues(medianIndex);
            filteredImage(i,j,k) = medianValue;
            end
        end
    end
